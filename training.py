...
class TrainingResult:
    def __init__(self, training_data_list : list):
        ...
        self.__best_config_time = (max(training_data_list, key = lambda item : np.mean(item.avg_time_list))).config
        self.__best_config_zscore = (max(training_data_list, key = lambda item : np.mean(item.zscore_list))).config

        self.__five_best_times = [x.config for x in heapq.nsmallest(5, training_data_list, key = lambda item : np.mean(item.avg_time_list))]
        self.__five_best_results = [x.config for x in heapq.nsmallest(5, training_data_list, key = lambda item : np.mean(item.zscore_list))]
...
class TrainingData:
    def __init__(self, method_name, dataset_name, config, avg_results_list, avg_time_list, zscore_list):
...
def training(meta_heuristic : function, dataset : ds.Dataset, hyper_param_list : list, method_name : str, dataset_name : str) -> TrainingResult:
    data = []
    for config in hyper_param_list:
        ...
        for k in dataset.training_ks:
            ...
            time_list = []
            for _ in range(10):
                result, time = meta_heuristic(dataset.points_data, dataset.points_data)
        ...
        data.append(TrainingData(method_name, dataset_name, config, avg_results_list, avg_time_list, zscore_list))
